...
def _reset_metrics_counters():
    """SP6 task 5: `atlas.metrics`'s request counter is process lifetime state (mirrors
    `accounts._STATE` above), so any test that boots `create_app()` and drives a request through it
    (test_chat_app.py's own `_server_app` helper does this repeatedly) would otherwise leak counts
    into whichever test happens to import `atlas.metrics` next. SP8 Task 4 remainder: the judge
    verdict counter pair is the same kind of process lifetime state (every `judge.emission.
    emit_verdict` call anywhere in the suite, including `contract_tools.trace_inventory`'s golden
    inventory collection, increments one of them), reset here for the identical reason."""
