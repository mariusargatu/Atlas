...
contract (a versioned triple), the judge routed through the gateway (a model grading a model on
tape), the before/after agreement study (κ rises above the bar after one documented rubric
correction), the position bias gate that swaps the answer order, and the panel's disagreement signal.
"""
...
# ---- the order swap, routed through the gateway, detects a real position flip ----
