"""Reranking + fusion, hermetic: make the reranker earn its place. The reranker
reorders the first pass; the test is whether it helps, measured as a with/without NDCG delta with a
recall guardrail, and whether the lift beats run-to-run noise (a paired test, ``quality.stats``, not a
glance at two averages). RRF (`atlas.domain.retrieval.rrf_fuse`, the ONE implementation in this repo,
shared with the pgvector adapter's own two arm fusion) is the deterministic hybrid whose job is to
never underperform its inputs. Reranker scores are replayed from a cassette (a real cross-encoder is deferred to dev/prod),
...
from atlas.domain.retrieval import RetrievalConfig, rrf_fuse
from atlas.ports.knowledge import Chunk
from evals.datasets.retrieval_golden import RETRIEVAL_CORPUS, RETRIEVAL_GOLDEN
from quality.ir_metrics import ndcg_at_k, recall_at_k
from quality.stats import paired_bootstrap_diff, paired_permutation_test
...
def test_cassette_reranker_reorders_by_recorded_score_deterministically():
    ...
    chunks = [Chunk(chunk_id="a", doc_id="a", text="a"), Chunk(chunk_id="b", doc_id="b", text="b"), Chunk(chunk_id="c", doc_id="c", text="c")]
...
def test_rrf_never_underperforms_either_single_backend():
    ...
    lexical = ["d1", "d2", "d3", "d4"]
    dense = ["d3", "d1", "d5", "d2"]
    fused = [doc_id for doc_id, _ in rrf_fuse([lexical, dense])]
...
def test_rrf_tie_break_is_id_ordered_not_insertion_ordered():
    # b and a get identical scores (1/61); the id tie-break must decide, yielding ['a','b'] and NOT
    # the ['b','a'] a score-only sort would leave in insertion order.
    assert [d for d, _ in rrf_fuse([["b"], ["a"]])] == ["a", "b"]
...
def _rerank_experiment():
    ...
    for case in RETRIEVAL_GOLDEN:
        first = retriever.search_chunks(case.query, k=5, config=RetrievalConfig())
...
def test_the_lift_is_inside_the_noise_at_this_sample_size():
    ...
    on this evidence, exactly the paired-test discipline this repo applies everywhere else."""
