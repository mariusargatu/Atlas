"""P4 (agent trajectory, hermetic): once the model can act, the final message stops being the thing
to test. The unit becomes the trajectory, the path of decisions (doc 08). This grades that path
...
async def test_a_read_turn_produces_a_sound_trajectory_with_no_write(build_replay_graph, seed_cassette, tmp_path):
    ...
    tool_text = serialize_tool_result({"customer": "Daniel", "plan": "Value (legacy, discontinued)", "has_contract": True})
    ...
    assert out["final_response"] == "You are on the Value (legacy) plan."   # rendered (cassette resolved)
    traj = trajectory_from_spans(tracer.spans, session_customer_id="cust_legacy_term", final_response=out["final_response"])
    # out-of-band oracle for a read: the answer actually names the customer's plan (not merely non-empty)
    report = grade_trajectory(traj, budget=_BUDGET,
                              goal_met="value" in (out["final_response"] or "").lower())
