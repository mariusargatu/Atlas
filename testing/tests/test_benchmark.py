...
The blog component embeds these exact numbers, so the study is pinned here: a seeded run is a
...
def test_study_numbers_match_the_committed_artifact(study_result):
    # The exact values the blog component is kept in sync with. A drift here is a drift in the post.
    r = study_result
