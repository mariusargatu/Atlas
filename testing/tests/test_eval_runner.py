...
live elsewhere. Here the grader is the trivial ``PredicateGrader``. The report itself is
held to the same law as everywhere else: no rate ships without its interval, and the gate reads the
floor.
"""
...
@pytest.mark.asyncio
async def test_eval_lane_drives_under_the_production_recursion_limit(tmp_path):
    """Finding 2: the graded eval lane must exercise the graph under the EXACT superstep ceiling the
    product edge runs, not langgraph's looser default. The runner drives every turn through the shared
    `thread_config`, so the config it passes carries recursion_limit == RECURSION_LIMIT, the same value
    chat_app's `_thread` produces. A bound only the product edge honoured would be an untested setting."""
    from atlas.domain.budget import RECURSION_LIMIT
    ...
    case = EvalCase(id="cfg", turns=("hi",), customer_id="cust_current")
    await _drive(_Graph(), case, thread_id="cfg-trial0")

    # the runner passes the production bound, and it is the SAME shape the product edge builds
    assert seen_configs and seen_configs[0]["recursion_limit"] == RECURSION_LIMIT
    assert thread_config("cfg-trial0")["recursion_limit"] == RECURSION_LIMIT


# ---- the grader stack (machinery only, concrete graders live elsewhere) ----
