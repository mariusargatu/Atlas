"""Read-surface injection, hermetic: the knowledge layer is the indirect prompt-injection
surface, the one input the agent reads rather than the user typing. The invariant that does
...
def test_poison_docs_are_retrieved_as_ordinary_data():
    corpus = [d.as_chunk() for d in POISON_CORPUS] + [
        Chunk(chunk_id="benign-router", doc_id="benign-router", text="router restart help holding the power button"),
    ]
    hits = InMemoryRetriever(corpus).search_chunks("reset equipment now", config=RetrievalConfig())
    assert any(c.doc_id.startswith("poison-") for c in hits)  # the poison is retrieved — as data, not a command
...
def test_semantic_cache_never_serves_one_customer_plan_answer_to_another():
    """The semantic-cache caution as a unit test: a plan-specific answer is keyed per customer,
