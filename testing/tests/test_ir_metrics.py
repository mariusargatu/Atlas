"""P1 (retrieval IR metrics, hermetic): the classical, judge-free retrieval metrics computed over
...
from evals.retrieval.ir_metrics import (
...
def _retrieved_ids(retriever: InMemoryRetriever, query: str, k: int) -> list[str]:
    return [c.doc_id for c in retriever.search(query, k=k)]
