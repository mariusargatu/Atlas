"""P3 (graph RAG, hermetic): when retrieval runs over a graph it stops being similarity search and
becomes traversal, which fails in ways a flat document set cannot (doc 07). Three deterministic
...
arithmetic on stable ids, so a wrong extraction or a hop-short traversal fails a `assert`, not a
...
from evals.retrieval.graph_metrics import bcubed_prf, pairwise_prf, path_recall, triple_prf
...
def test_the_relational_query_needs_a_graph_edge_that_a_keyword_vector_misses_even_at_deployed_k():
    ...
    got1 = [c.doc_id for c in retriever.search(RELATIONAL_QUERY, k=1)]
    got3 = [c.doc_id for c in retriever.search(RELATIONAL_QUERY, k=3)]
