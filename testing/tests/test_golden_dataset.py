"""The golden dataset layer: access and metadata slicing.

These pin the dataset machinery that surrounds the seed: the intermediary CSV and the
canonical seed describe the same set, the metadata is sliceable, and the coverage adds up. Scoring
the cases is deliberately deferred to structured-claim extraction. Silver to gold promotion and
decontamination land with the production loop. This stops at the dataset boundary.
"""
