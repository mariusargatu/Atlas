"""The golden dataset's two shapes: a LOOSE draft you ingest, a STRICT case you commit (04).
...
- ``GoldenDraft`` — what a bootstrap source (the CSV, a one-off script) yields. Only the SME core is
  required (who, what they asked, what correct means); every engineer-added field is optional, so a
  half-authored case still loads instead of failing the import. This is the "optional types on inject".

- ``GoldenCase`` — the canonical record. Every field is required and validated in ``__post_init__``:
  the enums must be in range, the identity must name a real seeded account, the turns must be
  non-empty. An invalid case cannot be constructed, so the committed seed (``datasets/seed.py``) is
...
turning that prose into an executable check against the account+catalog is the metrics article's
grader (05), deliberately not built in the dataset layer.
"""
...
@dataclass(frozen=True)
class GoldenCase:
    """The canonical, validated dataset record. All fields required; invalid construction raises.
