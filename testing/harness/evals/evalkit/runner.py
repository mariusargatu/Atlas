...
eval's job. (Turning that rate into a confidence interval is `quality.stats`'s job.)
...
async def _drive(graph, case: EvalCase, thread_id: str) -> str:
    ...
    plugs in exactly here and is deliberately not built in this layer. See ``case.py``.
    """
    final = ""
    # drive under the SAME config the product edge uses (recursion limit tied to the call budget), so
    # the eval lane exercises the graph at the exact superstep ceiling production runs (finding 2).
