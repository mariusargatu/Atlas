"""Access the canonical golden set, and slice it by the metadata that makes it debuggable (04).
...
same emergency. These are the dataset-level accessors that payoff. Scoring the slices is the metrics
article's job (05); this module hands it a set it can already cut by any facet.

The seed is all gold today. Silver-to-gold promotion (and the ``gold_only`` gate it needs) lands with
the production living loop (12), where sampled real traffic enters as silver and is promoted on
...
# The facets a regression is sliced by. ``risk`` is the CTO's roll-up; ``category`` the surface;
# ``consequence`` the weight; ``source``/``author_role``/``tier`` the provenance.
...
def as_eval_cases(cases: tuple[GoldenCase, ...] = GOLDEN) -> tuple[EvalCase, ...]:
    """Project the set to the runner's input. This is the seam the metrics article runs against:
    everything later in the series scores THESE cases, and this is how they reach the runner."""
