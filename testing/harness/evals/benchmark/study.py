...
`evals.stats` and `evals.gate`. Nothing is asserted. The result is a plain dict so `__main__`
can render it to both a human readable artifact and a JSON the blog component reads its
numbers from.
"""
...
def render(r: dict) -> str:
    """Render `run()`'s result to the human readable report the artifact/blog embed.
