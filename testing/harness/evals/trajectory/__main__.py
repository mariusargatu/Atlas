...
so its DISCRIMINATION is visible (completed scores high, refused low) — a single success fixture would
not prove the metric has teeth. ``goal_completed`` is an OUT-OF-BAND oracle: the account's actual plan
after the run, read from the store, never the trajectory's own write_applied flag.
...
async def _run_agent(sc: _Scenario) -> _Run:
    ...
    with tempfile.TemporaryDirectory() as cassette_dir:
        ...
        cfg = {"configurable": {"thread_id": f"traj-{sc.name}"}}
