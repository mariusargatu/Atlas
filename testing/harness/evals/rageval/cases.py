...
metric will (correctly) score it high, which is exactly the point, faithfulness keeps the model
honest about its sources, and only Atlas's own deterministic correctness-vs-oracle check
(``domain.metrics.is_correct_vs_truth``) catches that the source was the wrong one.
"""
