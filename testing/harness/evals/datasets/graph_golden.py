...
clothes (doc 07). A legacy customer, her plan, its data-cap term, the throttling that enforces it,
and the regional exception that also reaches the throttling clause, exactly the chain a single vector
lookup is structurally prone to fumble and a graph is good at.
...
# no legitimate reasoning path, so a traversal reads it without obeying it (doc 07's graph poison).
...
RELATIONAL_CORPUS: list[Chunk] = [
    Chunk("throttle-clause", "A capped connection is throttled to a lower speed once its allowance is spent."),
    Chunk("saver-plan", "The legacy Saver plan carries a monthly allowance and a minimum contract term."),
    Chunk("router-help", "Restart the router by holding its power button for ten seconds."),
    Chunk("billing-help", "Invoices are raised monthly on the anniversary date."),
    Chunk("outage-help", "Late fees are waived during a confirmed network outage."),
]
