...
customer). The write surface and the adversarial cases carry the weight. ``oracle`` is a prose
pointer to the source of truth; the grader that makes it executable is elsewhere in this package.
``tier`` is ``gold`` throughout. Silver/generated volume enters a separate file and is promoted in
only after review.
"""
