"""Information retrieval metrics: the judge-free half of RAG evaluation ("retrieval first").
...
Pure stdlib by design, the same discipline as ``quality.stats``: the interval math and the ranking
...
def rank_overlap_at_k(a: Sequence[str], b: Sequence[str], k: int) -> float:
    """Structural agreement between two RANKINGS over the same query space: the fraction of `k`
    that the top-k id SETS of `a` and `b` share, `|top_k(a) & top_k(b)| / k`. Unlike every other
    function in this module, neither argument is a relevance judgment; both are retrieval outputs.
    This is the metamorphic/perturbation primitive (a rewording, a typo, a casing change should not
    wildly change WHICH ids come back, independent of whether any of them are actually relevant): a
    paraphrase family's invariant is that its members' retrieved sets stay close to each other,
    which this measures directly, id set to id set.

    Symmetric in `a`/`b` (set intersection is symmetric). Divides by `k`, the same trec_eval P@k
    convention `precision_at_k` already uses, so a ranking shorter than `k` is penalised for the
    missing positions rather than flattered by a smaller denominator."""
    _validate_k(k)
