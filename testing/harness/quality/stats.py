...
under the same `task test` as everything else. The heavier frontier (the NIST GLMM via
statsmodels, PPI via ppi-py) is deliberately deferred to a dev/prod extra rather than
dragged into the hermetic core.
...
provenance. A bootstrap whose seed is not recorded does not ship.
"""
...
def cohen_kappa_interval(
    ...
    the normal approximation loose, exactly where a bootstrap on kappa is the honest tool
    (deliberately not built here) but the floor is still the right thing to gate on.
    """
    n = len(rater_a)
    if n == 0 or len(rater_b) != n:
        raise ValueError("raters must be equal length and not empty")
...
def gwet_ac1(rater_a: list[int], rater_b: list[int]) -> float:
    """Gwet's AC1 (2008): a chance corrected agreement coefficient built for the same problem kappa
    is famous for mishandling, prevalence. Kappa's expected agreement term is built from each rater's
    OWN marginal (how often each one says "yes"), so it collapses toward the observed agreement, and
    kappa toward zero, whenever almost every case shares one label, even when the raters plainly agree
    on the hard ones too (the "kappa paradox", Cicchetti and Feinstein 1990, Gwet 2008). AC1 replaces
    that marginal based chance model with one keyed on the label's average prevalence, a difference
    that keeps AC1 stable at exactly the skew that punishes kappa. Reported alongside kappa (D15),
    never instead of it, the same "read the numbers together, trust neither alone" discipline this
    module already holds `cohen_kappa`/`raw_agreement` to.
    """
    n = len(rater_a)
    if n == 0 or len(rater_b) != n:
        raise ValueError("raters must be equal length and not empty")
    observed = sum(1 for x, y in zip(rater_a, rater_b) if x == y) / n
    prevalence_mean = (sum(rater_a) / n + sum(rater_b) / n) / 2
    expected = 2 * prevalence_mean * (1 - prevalence_mean)
    if expected >= 1.0:
        return 1.0
    return (observed - expected) / (1 - expected)
...
def holm_bonferroni(p_values: Sequence[float]) -> list[float]:
    """Holm step down adjustment for multiple comparisons, returned in the SAME order as the input.

    Comparing many metrics (a benchmark matrix row per model) inflates the false positive rate: run
    twenty independent tests at alpha 0.05 and roughly one clears the bar by chance alone. Holm
    controls the family wise error rate without Bonferroni's full conservatism (dividing every
    p value by m): only the smallest p pays the full m penalty, the next pays m - 1, and so on, so
    a real effect buried among many null ones is not punished for its neighbours' nulls.

    Sort ascending, multiply p_(rank) by (m - rank) [0 indexed rank, so the smallest pays the full
    m], cap each at 1.0, then take a running maximum so the adjusted sequence is never decreasing in
    sorted order. That last step is not an implementation detail: without it a later, larger p could
    adjust to something smaller than an earlier one, which would let a weaker result look stronger
    after correction than a stronger one, backwards. Matches the standard Holm adjusted p value
    convention (R's `p.adjust(method="holm")`, `statsmodels.stats.multitest.multipletests`).

    At m=1 this degenerates to the raw p value: nothing to correct for when there is only one test,
    the honest floor a multiple comparison correction should reduce to.
    """
    m = len(p_values)
    if m == 0:
        raise ValueError("holm_bonferroni needs at least one p value")
    for p in p_values:
        if not 0.0 <= p <= 1.0:
