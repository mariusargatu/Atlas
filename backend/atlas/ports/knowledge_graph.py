...
The CI adapter is an in-memory graph over a hand-authored gold KG (deterministic, no DB). A real
``Neo4jKnowledgeGraph`` (the ``neo4j-graphrag`` retrievers) sits behind this same port in the
operator lane; there is no pure-Python embedded Neo4j, so the in-memory adapter is not a shortcut,
it is the only way to keep graph RAG in the hermetic gate. Everything a metric grades is a stable id.
"""
