...
type LabelItemsOut = components["schemas"]["LabelItemsOut"];
type LabelOut = components["schemas"]["LabelOut"];
...
export const handlers = [
  ...
  http.post(`${base}/auth/step-up`, () =>
    HttpResponse.json({
      access_token: "test-access-write",
      customer_id: "cust_current",
      name: "Test User",
    }),
  ),
  ...
  http.get(`${base}/labels/items`, () => {
    const out: LabelItemsOut = {
      items: [
        {
          case_id: "fixture-1",
          trace_id: "t1",
          ...
          answer: "Yes.",
          retrieved_chunks: [
            { doc_id: "d1", chunk_id: "d1", text: "contract-free plan", score: 0 },
          ],
        ...
        },
        {
          ...
          question: "Data cap?",
          answer: "No cap.",
          retrieved_chunks: [{ doc_id: "d2", chunk_id: "d2", text: "unlimited data", score: 0 }],
          registry_facts: [{ fact_id: "f2", value: "unlimited" }],
        },
      ],
      progress: { labeled: 0, total: 2 },
    };
    return HttpResponse.json(out);
  }),
  http.post(`${base}/labels`, async ({ request }) => {
    const { critique } = (await request.json()) as { critique: string };
    if (!critique || !critique.trim()) {
      return HttpResponse.json({ detail: "critique is required" }, { status: 422 });
    }
    const out: LabelOut = { progress: { labeled: 1, total: 2 } };
    return HttpResponse.json(out);
  }),
